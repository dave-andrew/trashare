...
export default function WasteTypeSelector({wasteType, setWasteType}: {wasteType: string, setWasteType: (type: string) => void}) {
  ...
  return (
    <View className='flex flex-row gap-1'>
      ...
      <Pressable
        className={`w-12 h-12 ${wasteType == 'Recyclable' ? 'bg-[#00B1F7]' : 'bg-[#a0a0a0]'} flex items-center justify-center rounded-full`}
        onPress={() => setWasteType('Recyclable')}>
        <FontAwesome name='recycle' size={20} color={"#fff"} />
      </Pressable>

      <Pressable
        className={`w-12 h-12 ${wasteType == 'Compost' ? 'bg-green-600' : 'bg-[#a0a0a0]'} flex items-center justify-center rounded-full`}
